...
description: Glase Merchant, webshop, POS, service, in-app integration
---


# Glase payment in your app 

To integrate your app with Glase, you need to add Glase payment in your “Check out” process where your customer normally selects between payment methods. 

All apps offering payments typically have a backend that handles the actual service/product that is sold. It is this backend that must connect to Glase using our interface, and the communication between your app and your backend is up to your app developer.

### Example of shopping and payment flow in your app and Glase app
...
Follow these steps to integrate your app with Glase:
...
The flow for an app Glase payment is very similar to [Basic Glase payment](/merchant/payment).
The difference is that the user presses a "pay" button or link in your app, which launches the Glase app and presents the bill for the user to confirm the payment.
...
If user will press cancel in Glase app then invoice will get canceled. The backURL will still be called.
...
Make a redirection (button or link) in your app, which launches the Glase app, using the QR code URL returned from the sendInvoice request: Replace the "HTTP:" header with "seqr:" that is, if sendInvoice returns HTTP://SEQR.SE/R12345, the button/link should instead use seqr://SEQR.SE/R12345 .



### Present the receipt

Once the payment is completed, your app should query the status of the invoice from Glase by calling getPaymentStatus. If the invoice was successfully paid, a reference number (ersReference) is obtained from 
Glase. Save the reference number for follow-ups and for print on the user 
